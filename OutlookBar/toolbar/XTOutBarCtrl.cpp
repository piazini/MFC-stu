...
// ?998-2002 Codejock Software, All Rights Reserved.
//
   ...
   /* if (m_arFolder.GetSize() != 0)
    {
        ...
        case F_NORMAL:
            {
				...
				if (m_pFolderImageList)
				{
					...
					CSize szImage;
					IMAGEINFO ii;
					m_pFolderImageList->GetImageInfo(iIndex, &ii);
					szImage = CRect(ii.rcImage).Size();
				
					pt.x = rcFolder.left +rcFolder.Width()/16;
					pt.y = rcFolder.top + (rcFolder.Height() - szImage.cy) / 2;
					m_pFolderImageList->Draw(pDC, iIndex, pt, ILD_NORMAL);
				}
				//////////
		...
		pnFolderBorder.CreatePen(PS_SOLID,1,RGB(102,188,230));
		...
		for(int i=0;i<64;i++)
		{
			br[i].CreateSolidBrush(RGB((221-i/2),(242-i/2),(255)));
		}
		...
		pDC->DrawText(pBarFolder->m_strName,rect,DT_CENTER|DT_VCENTER|DT_SINGLELINE);
    ...
    if (ht == ITEM)
    {
        ...
        if (::GetCapture() == NULL)
        {
            ...
            for (;;)
            {
              ...
              //xu                  ::SetCursor(xtAfxData.hcurDragMove);
              //xu                 hCur = xtAfxData.hcurDragMove;
                            }
                            else 
                            {
                                CRect rcItem;
...
//xu                                    ::SetCursor(xtAfxData.hcurDragNone);
//xu                                    hCur = xtAfxData.hcurDragNone;
                                }
                            }
                        }
                        else
        ...
        if (IsSmallIconView(iFolder))
        {
            if (pImageList)
            {
               /* IMAGEINFO ii;
				...
				Image image(L"left_bg01.gif", FALSE);
				CPoint pt;
                pt.x = rc.left + 2;
                pt.y = rc.top + (rc.Height() - image.GetHeight()) / 2;
				graphics.DrawImage(&image, pt.x,pt.y,rc.Width(),image.GetHeight());


                if (!bOnlyImage)
                {
                    //rc.left  += (/*szImage.cx*/48 + m_sizeOffset.cx);
...
BOOL CXTOutBarCtrl::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
    ...
    if (ht == FOLDER)
    {
//xu        ::SetCursor(xtAfxData.hcurHand);
        return TRUE;
    }
...
//		coolMenu.LoadToolbar(XT_IDR_TBAR_OUT);
		...
		CMenu menu;
//xu		VERIFY(menu.LoadMenu(XT_IDM_POPUP));
		menu.CreatePopupMenu();
		menu.AppendMenu(MF_ENABLED,XT_IDC_SMALLICON,"小图标");
		menu.AppendMenu(MF_ENABLED,XT_IDC_LARGEICON,"大图标");
		
//xu		CMenu* pPopup = menu.GetSubMenu(0);
//xu		ASSERT(pPopup != NULL);
//xu		CWnd* pWndPopupOwner = this;
...
void CXTOutBarCtrl::SetFolderImageList(CImageList *pImageList)
{
